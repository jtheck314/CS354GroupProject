...
mean.stdv.CR <- as.data.frame(rjson::fromJSON(file = "StdDev/MeanStdDevCR.json"))

mean.stdv.Country <- as.data.frame(rjson::fromJSON(file = "StdDev/MeanStdDevCountry.json"))

mean.stdv.Classical <- as.data.frame(rjson::fromJSON(file = "StdDev/MeanStdDevClassical.json"))
...
energyProb <- function(energyValue = 0.0){dnorm(x=energyValue,mean=0, sd=1)}
...
#result <- classify(CRPrior = (1/3),CountryPrior = (1/3),ClassicalPrior = (1/3), song = songCountry)